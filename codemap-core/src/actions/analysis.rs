...
// summary, decorators, rename, context moved to insights.rs
...
pub fn health(graph: &Graph) -> String {
    ...
    for node in graph.nodes.values() {
        for f in &node.functions {
            if f.calls.len() > 15 { high_complexity_count += 1; }
        }
    }
    ...
    let grade = match total_score {
        90..=100 => "A", 80..=89 => "B", 65..=79 => "C", 50..=64 => "D", _ => "F",
    };
    ...
    if total_score < 80 {
        ...
        if cycle_score < 20 { lines.push(format!("  - Break {} circular dependencies (codemap circular)", cycle_count)); }
        if coupling_score < 20 { lines.push("  - Reduce coupling in god files (codemap hotspots)".to_string()); }
        if dead_score < 20 { lines.push("  - Remove dead files (codemap dead-files)".to_string()); }
        if complexity_score < 20 { lines.push("  - Simplify complex functions (codemap complexity .)".to_string()); }
    }
